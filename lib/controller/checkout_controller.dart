...
import '../backend/model/offers_model.dart';
import '../backend/model/payment_model.dart';
import '../backend/parse/checkout_parse.dart';
import 'address_list_controller.dart';
import 'booking_controller.dart';
import 'cart_controller.dart';
import 'coupens_controller.dart';
import 'tabs_controller.dart';
import 'web_payment_controller.dart';
import '../env.dart';
...
class CheckoutController extends GetxController implements GetxService {
  ...
  Future<void> getFreelancerByID() async {
    var response = await parser.getFreelancerByID({"id": Get.find<CartController>().savedInCart[0].uid});
    apiCalled = true;
    if (response.statusCode == 200) {
      Map<String, dynamic> myMap = Map<String, dynamic>.from(response.body);
  ...
  void calculateDistance() {
    ...
    if (addressInfo.lat != null && addressInfo.lng != null && freelancerDetail.lat != null && freelancerDetail.lng != null) {
      ...
      if (distance > Get.find<CartController>().parser.getAllowedDeliveryRadius()) {
        haveFairDeliveryRadius = false;
        showToast('${'Sorry we deliver the order near to'.tr} ${Get.find<CartController>().parser.getAllowedDeliveryRadius()} KM');
      } else {
        if (Get.find<CartController>().shippingMethod == 0) {
          double distancePricer = distance * Get.find<CartController>().shippingPrice;
  ...
  void calculateAllCharge() {
    double totalPrice = Get.find<CartController>().totalPrice + Get.find<CartController>().orderTax + deliveryPrice;
    if (_selectedCoupon.discount != null && _selectedCoupon.discount != 0) {
      double percentage(numFirst, per) {
        return (numFirst / 100) * per;
      }

      _discount = percentage(Get.find<CartController>().totalPrice, _selectedCoupon.discount); // null
    }
  ...
  void onCoupens(String offerId, String offerName) {
    ...
    Get.toNamed(AppRouter.getcouponRoute(), arguments: ['service', offerId, offerName]);
  }
  ...
  void onSelectAddress() {
    ...
    Get.toNamed(AppRouter.getAddressListRoute(), arguments: ['service', selectedAddressId]);
  }
  ...
  void onDeleteService(int index) {
    Get.find<CartController>().deleteFromList(Get.find<CartController>().savedInCart[index]);
    calculateAllCharge();
    update();
  }
